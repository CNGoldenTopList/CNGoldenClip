...
(() => {
  ...
  // node_modules/pinyin-pro/dist/esm/data/special.mjs
  var InitialList = [
    "zh",
    "ch",
    "sh",
  ...
  var PatternNumberDict = Object.keys(NumberDict).map((key) => ({
    zh: key,
  ...
  // node_modules/pinyin-pro/dist/esm/core/custom/state.mjs
  var customMultipleDict = new FastDictFactory();
  var customPolyphonicDict = new FastDictFactory();

  // node_modules/pinyin-pro/dist/esm/data/surname.mjs
  var Surnames = {
    南宫: "nán gōng",
    第五: "dì wǔ",
    万俟: "mò qí",
  ...
  var PatternSurname = Object.keys(Surnames).map((key) => ({
    zh: key,
    pinyin: Surnames[key],
  ...
  // node_modules/pinyin-pro/dist/esm/core/pinyin/all.mjs
  var getAllPinyin = (char, surname = "off") => {
    let pinyin2 = DICT1.get(char) ? DICT1.get(char).split(" ") : [];
    if (customMultipleDict.get(char)) {
      pinyin2 = customMultipleDict.get(char).split(" ");
    } else if (surname !== "off") {
  ...
  // node_modules/pinyin-pro/dist/esm/data/dict2.mjs
  var DICT2 = {
    这个: "zhè ge",
    成为: "chéng wéi",
    认为: "rèn wéi",
  ...
  var Pattern2 = Object.keys(DICT2).map((key) => ({
    zh: key,
    pinyin: DICT2[key],
    probability: 2e-8,
    length: 2,
  ...
  // node_modules/pinyin-pro/dist/esm/data/dict3.mjs
  var DICT3 = {
    为什么: "wèi shén me",
    实际上: "shí jì shang",
    检察长: "jiǎn chá zhǎng",
  ...
  var Pattern3 = Object.keys(DICT3).map((key) => ({
    zh: key,
    pinyin: DICT3[key],
    probability: 2e-8,
    length: 3,
  ...
  // node_modules/pinyin-pro/dist/esm/data/dict4.mjs
  var DICT4 = {
  ...
  var Pattern4 = Object.keys(DICT4).map((key) => ({
    zh: key,
    pinyin: DICT4[key],
    probability: 2e-8,
    length: 4,
  ...
  // node_modules/pinyin-pro/dist/esm/data/dict5.mjs
  var DICT5 = {
  ...
  var Pattern5 = Object.keys(DICT5).map((key) => ({
    zh: key,
    pinyin: DICT5[key],
    probability: 2e-8,
    length: 5,
  ...
  // node_modules/pinyin-pro/dist/esm/data/patterns.mjs
  var PatternsNormal = [
    ...Pattern5,
    ...Pattern4,
    ...Pattern3,
  ...
  // node_modules/pinyin-pro/dist/esm/common/segmentit/max-probability.mjs
  ...
  // node_modules/pinyin-pro/dist/esm/common/segmentit/min-tokenization.mjs
  ...
  // node_modules/pinyin-pro/dist/esm/common/segmentit/reverse-max-match.mjs
  ...
  // node_modules/pinyin-pro/dist/esm/common/segmentit/index.mjs
  ...
  var AC = class {
    ...
    match(text, surname, zhChars = splitString(text)) {
      ...
      for (let i = 0; i < zhChars.length; i++) {
        let c = zhChars[i];
        let next = cur.children.size ? cur.children.get(c) : void 0;
        while (!next && cur !== this.root) {
          ...
          cur = cur.fail;
          next = cur.children.size ? cur.children.get(c) : void 0;
        }
        if (next) {
          cur = next;
  ...
  // node_modules/pinyin-pro/dist/esm/core/traditional/index.mjs
  var traditionalDict = [];
  function getTraditionalDict() {
    return traditionalDict;
  }

  // node_modules/pinyin-pro/dist/esm/core/pinyin/handle.mjs
  var getSingleWordPinyin = (char) => {
    const pinyin2 = DICT1.get(char);
    if (!pinyin2)
      return char;
    const index = pinyin2.indexOf(" ");
    return index === -1 ? pinyin2 : pinyin2.slice(0, index);
  };
  var getTraditionalWords = (word) => {
    const traditionalWords = [];
    const traditionalDict2 = getTraditionalDict();
    for (let i = 0; i < word.length; i++) {
      const key = word[i];
      const code = key.charCodeAt(0);
      if (traditionalDict2[code]) {
        traditionalWords[i] = traditionalDict2[code];
      } else {
  ...
  var getMultiplePinyin = (word, surname = "off") => {
  ...
  var getInitialAndFinal = (pinyin2, initialPattern) => {
  ...
  var getFirstLetter = (pinyin2, isZh) => {
  ...
  // node_modules/pinyin-pro/dist/esm/core/pinyin/middlewares.mjs
  var validateType = (word) => {
  ...
  // src/vendor/search.ts
  function normalizeSearch(value) {
    return value.toLowerCase().replace(/[\s·•._()（）\-—–/\\]+/g, "").trim();
  }
  function implicitPinyin(value) {
  ...
  function searchable(values, aliases, query) {
    const q = normalizeSearch(query);
    if (!q) return true;
    const queries = /[\u3400-\u9fff]/.test(q) && /[a-z]/.test(q) ? [q, ...implicitPinyin(q).map(normalizeSearch)] : [q];
    return [...values.filter(Boolean), ...aliases ?? []].some(
      (value) => [value, ...implicitPinyin(value)].some((candidate) => queries.some((query2) => normalizeSearch(candidate).includes(query2)))
    );
  }

  // src/vendor/labels.ts
  var challengeDisplayName = (challenge) => challenge.name.replace(/\[(C\/FC|FC|C)\]\s*$/i, "$1");

  // src/core.mjs
  var statusNames = { pending: "待审核", accepted: "已通过", hidden: "隐藏", rejected: "已拒绝" };
  function videoRef(raw) {
    try {
  ...
  function beijingDate(value) {
    ...
    return Number.isFinite(time) ? new Date(time + 8 * 36e5).toISOString().slice(0, 10) : "";
  }
  function fullCardDate(text) {
    const match = text?.match(/(\d{4})[年/-](\d{1,2})[月/-](\d{1,2})/);
    return match ? validDate(`${match[1]}-${match[2].padStart(2, "0")}-${match[3].padStart(2, "0")}`) : "";
  }
  function playerUids(player) {
  ...
  function mergeRecords(publicRecords, adminRecords) {
    return [...new Map([...publicRecords.map((r) => ({ ...r, status: r.status || "accepted" })), ...adminRecords].map((r) => [r.id, r])).values()];
  }
  function duplicates(records, playerId, challengeId, url) {
    const key = videoRef(url)?.key;
    return records.filter((r) => r.playerId === playerId && r.challengeId === challengeId).map((r) => ({ ...r, sameVideo: Boolean(key && videoRef(r.videoUrl)?.key === key) }));
  }
  function submissionBody({ player, challenge, videoUrl, achievedAt, rawVideoUrl = "", playerNote = "" }) {
  ...
  function readCard(card) {
    const link2 = card.querySelector('a[href*="/video/BV"]');
    const video = videoRef(link2?.getAttribute("href"));
  ...
  var standardOrder = ["high-std", "mid-std", "low-std"];
  var standardMeta = {
    "high-std": { label: "High Std", color: "#d4d4d4" },
    "mid-std": { label: "Mid Std", color: "#adadad" },
    "low-std": { label: "Low Std", color: "#868686" }
  };
  var ratedTierOrder = [...tierOrder, ...standardOrder];
  var isTierCode = (code) => Boolean(code && Object.hasOwn(tierMeta, code));
  var isStandardTier = (code) => Boolean(code && Object.hasOwn(standardMeta, code));
  var isRatedTier = (code) => isTierCode(code) || isStandardTier(code);
  var tierBadgeLabel = (code) => isStandardTier(code) ? standardMeta[code].label : tierMeta[code].label.replace(/^(High|Mid|Low) Tier /, "$1 T");
...
`;
  ...
  function link(label, url) {
    ...
    try {
      ...
      if (/^https?:$/.test(parsed.protocol)) a.href = parsed.href;
    } catch {
    }
    return a;
  }
  function mount() {
    const host = el("div");
    host.setAttribute("data-cngist-ui", "");
    const root = host.attachShadow({ mode: "open" });
    root.append(el("style", css));
    document.body.append(host);
    return root;
  }
  function loadVideoDate(gm2, video) {
    return new Promise((resolve, reject) => gm2.xmlhttpRequest({
      method: "GET",
      url: video.url,
      timeout: 15e3,
      onload: (response) => {
      ...
      onerror: () => reject(new Error("视频页面请求失败")),
  ...
  async function start({ gm: gm2, origin = "https://cngist.com", role, searchable: searchable2, challengeName, videoLoader = loadVideoDate }) {
    ...
    const toolbar = el("div", void 0, { className: "toolbar" });
    ...
    gm2.registerMenuCommand("连接 CN 金榜管理员", () => connect.click());
    async function loadCatalog(force = false) {
      ...
      catalogPromise = client.request("catalog").then((data) => {
        ...
        choices = [...data.challenges, ...data.multiMapChallenges].map((c) => {
          const map2 = maps.get(c.mapId), pack = packs.get(c.campaignId || map2?.campaignId);
          const tier = isRatedTier(c.tier) ? tierBadgeLabel(c.tier).replace(/Tier /, "T") : "未定档";
          return {
            ...c,
            label: [names(pack), map2 && names(map2), challengeName(c), tier].filter(Boolean).join(" / "),
            search: [pack?.name, pack?.cnName, pack?.shortName, map2?.name, map2?.cnName, c.name, c.type, tier],
            aliases: [...pack?.searchAliases || [], ...map2?.searchAliases || []],
    ...
    function picker(labelText, placeholder, options, matches, name, onSelect) {
      const wrap = el("div", void 0, { className: "picker" });
      const label = el("label", labelText), input = el("input", void 0, { placeholder, type: "search", autocomplete: "off" });
      const list = el("div", void 0, { className: "results", hidden: true });
      const selected = el("div", "", { className: "selected" });
      label.append(input);
      wrap.append(label, list, selected);
      let value = null, cursor = -1, shown = [];
      const choose = (item) => {
        value = item;
        input.value = name(item);
        selected.textContent = name(item);
        list.hidden = true;
        onSelect(item);
      };
      function render() {
      ...
      input.onkeydown = (event) => {
        ...
        if (["ArrowDown", "ArrowUp"].includes(event.key)) {
          ...
          [...list.children].forEach((b, i) => b.classList.toggle("active", i === cursor));
          list.children[cursor]?.scrollIntoView({ block: "nearest" });
        }
        if (event.key === "Enter") {
          event.preventDefault();
          if (!list.hidden && shown.length) choose(shown[Math.max(0, cursor)]);
        }
      };
      return { wrap, input, selected, choose, get value() {
        return value;
      } };
    }
    async function openForm(video, button) {
      ...
      const panel = el("div", void 0, { className: "panel" });
      dialog.append(panel);
      root.append(dialog);
      ...
      const head = el("div", void 0, { className: "head" }), heading = el("div");
      ...
      const duplicateLine = el("div", void 0, { className: "dup", role: "status" });
      ...
      const recordList = el("div", void 0, { className: "records", hidden: true });
      ...
      const playerPicker = picker(
        "金榜玩家",
        "搜索名字或 B 站 UID",
        () => catalog.players,
        (p, q) => searchable2([p.name, ...playerUids(p)], p.aliases, q),
        (p) => `${p.name} · UID ${playerUids(p).join(" / ") || "未登记"}`,
        () => {
          void refreshRecords();
          update();
        }
      ...
      const challengePicker = picker(
        "已有挑战",
        "搜索地图包、地图、挑战；支持中文 / 拼音 / 英文",
        () => orderedChoices,
        (c, q) => q.split(/\s+/).every((part) => searchable2(c.search, c.aliases, part)),
        (c) => c.label,
        () => update()
      );
      ...
      urlLabel.append(url);
      grid.append(dateLabel, urlLabel);
      panel.append(grid);
      const extra = el("details", void 0, { className: "extra" });
      extra.append(el("summary", "RAW 链接与备注（可选）"));
      const rawLabel = el("label", "RAW 视频链接"), raw = el("input", void 0, { type: "url", placeholder: "https://" });
      rawLabel.append(raw);
      const noteLabel = el("label", "玩家备注"), note = el("textarea", void 0, { rows: 2, maxLength: 4e3 });
      noteLabel.append(note);
      extra.append(rawLabel, noteLabel);
      panel.append(extra, result);
      const footer = el("div", void 0, { className: "footer" });
      footer.append(policy, submit);
      panel.append(footer);
      ...
      function update() {
        ...
        if (!player || !goal) duplicateText.textContent = "选好玩家和挑战后显示重复提醒。";
          ...
          const counts = Object.entries(statusNames).map(([status, name]) => {
            const n = found.filter((r) => r.status === status).length;
            return n ? `${name} ${n} 条` : "";
          }).filter(Boolean).join("、");
        ...
        for (const r of found) {
          const row = el("div");
          row.append(el("span", `${statusNames[r.status] || r.status} · ${r.achievedAt || "日期未知"}${r.sameVideo ? " · 同一视频" : ""}`), link("视频", r.videoUrl), link("金榜记录", `${origin}/record/${r.id}`));
      ...
      async function refreshRecords() {
        ...
        recordWarning = "";
        loadingRecords = Boolean(player);
        refresh.disabled = Boolean(player);
        update();
        if (!player) {
        ...
        try {
          ...
          recordWarning = data.warnings.join("；");
        } catch (error) {
          if (version === checkVersion) recordWarning = error.message;
        } finally {
      ...
      submit.onclick = async () => {
        ...
        try {
          body = submissionBody({
            player: playerPicker.value,
            challenge: challengePicker.value,
            videoUrl: url.value,
            achievedAt: date.value,
            rawVideoUrl: raw.value.trim(),
            playerNote: note.value
          });
        } catch (error) {
        ...
        const fields = [...panel.querySelectorAll("input,textarea,button")];
        fields.forEach((f) => {
          f.disabled = true;
        });
        try {
      ...
      if (matched.length === 1) {
        playerPicker.choose(matched[0]);
        challengePicker.input.focus();
      } else {
        playerPicker.selected.textContent = matched.length > 1 ? "该 UID 对应多个档案，请管理员核对选择。" : "未找到该 UID 对应的金榜玩家，请核对或先在金榜建档。";
        playerPicker.selected.className = "selected warning";
        playerPicker.input.focus();
      }
      update();
      if (!video.date) {
        if (!dateCache.has(video.bv)) dateCache.set(video.bv, videoLoader(gm2, video).catch((error) => {
          dateCache.delete(video.bv);
          throw error;
        }));
        dateCache.get(video.bv).then((value) => {
          ...
          if (dialog.isConnected) {
            dateHint.textContent = "未能读取准确发布日期，请打开视频核对后填写。";
            dateHint.className = "hint warning";
          }
        });
      }
    }
    function scan() {
      for (const card of document.querySelectorAll(".bili-video-card, .video-item")) {
        ...
        host?.remove();
        host = el("div");
        host.dataset.cngistButton = "";
        host.dataset.video = video.key;
